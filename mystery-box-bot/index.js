...
async function spawnBox(debug = false) {
    ...
    if (debug) {
        console.log("DEBUG SPAWN CHANNEL ID:", channelId);
    }

    setTimeout(() => spawnBox(), getRandomInterval());
}
...
client.on("interactionCreate", async interaction => {
    ...
    if (interaction.isChatInputCommand()) {
        ...
        if (interaction.commandName === "testspawn") {
            ...
            const embed = new EmbedBuilder()
                .setTitle("🧪 TEST SPAWN")
                .setDescription("Testing random channel spawn system")
                .setColor("Blue");
            ...
            await interaction.reply({
                content: `✅ Test box spawned in: <#${channelId}>`,
                ephemeral: true
            });

            console.log("🧪 TEST SPAWN ->", channelId);
        }
    }
});
