...
function getRandomChannel() {
    const id = config.channels[Math.floor(Math.random() * config.channels.length)];
    return id;
}
...
client.on("interactionCreate", async interaction => {
    ...
    if (interaction.customId === "open_box") {
        ...
        rewardText = `${reward.type}: ${reward.value}`;

        const winEmbed = new EmbedBuilder()
            .setTitle("🎉 Mystery Box Opened!")
            .setDescription(`${interaction.user} opened the box and got **${rewardText}**!`)
            .setColor("Green");
